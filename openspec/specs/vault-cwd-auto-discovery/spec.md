# Vault CWD Auto-Discovery

## Purpose

Automatically resolve the effective vault from the current working directory at CLI startup, so users do not need to pass `--vault` when running commands from inside a vault directory tree.
