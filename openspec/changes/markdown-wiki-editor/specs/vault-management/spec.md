...
### Requirement: Remove vault
The system SHALL allow the user to remove a vault from the registry. Removing a vault SHALL only deregister it — the folder and its contents SHALL NOT be deleted from disk.

#### Scenario: Remove a registered vault
- **WHEN** the user removes a vault
- **THEN** the vault is removed from `vaults.json`
- **THEN** its cache directory under `$XDG_CACHE_HOME/cerbo/<vault-id>/` SHALL be deleted
- **THEN** the folder on disk remains untouched

### Requirement: Switch active vault
The system SHALL allow the user to switch between registered vaults. The active vault SHALL be persisted in `vaults.json` as `activeVaultId`.

#### Scenario: Switch to another vault
- **WHEN** the user selects a different vault
- **THEN** the UI displays the page list of the selected vault
- **THEN** `activeVaultId` is updated in `vaults.json`
