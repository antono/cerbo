...
The system SHALL discover the cerbo repository by walking up from the current working directory looking for a directory containing a `.cerbo/` subdirectory. The first such directory found is the repository root. If no `.cerbo/` is found before reaching the filesystem root or a mount-point boundary, the command SHALL abort with the message `not a cerbo repository (or any parent up to mount point)` written to stderr and exit non-zero. The command SHALL NOT accept a positional argument for vault selection.

#### Scenario: Run from repository root
- **WHEN** user runs `cerbo symlink` in a directory containing `.cerbo/`
- **THEN** that directory is treated as the repository root
- **THEN** the command proceeds against it

#### Scenario: Run from nested subdirectory
- **WHEN** user runs `cerbo symlink` from `<repo-root>/notes/rust/`
- **THEN** the command walks up, finds `.cerbo/` at `<repo-root>/`
- **THEN** the command proceeds with `<repo-root>/` as the root

#### Scenario: Run outside any cerbo repository
- **WHEN** user runs `cerbo symlink` from a directory with no `.cerbo/` in any ancestor
- **THEN** the command exits non-zero
- **THEN** stderr contains `not a cerbo repository (or any parent up to mount point)`
...
### Requirement: Fixed output location at `<repo-root>/cerbo/`

The system SHALL materialise the symlink tree at the fixed path `<repo-root>/cerbo/`. The output directory name SHALL be the literal string `cerbo`, independent of the repository's basename. The tree SHALL always live inside the repository (the same directory tree that contains `.cerbo/`).
...
- **THEN** `<repo-root>/cerbo/notes/rust/` is a single real directory
...
- **THEN** the page is symlinked under `<repo-root>/cerbo/`
- **THEN** the ontology object has no corresponding symlink anywhere in `<repo-root>/cerbo/`

### Requirement: Symlink target form

The system SHALL set each leaf symlink's target to the object directory `<repo-root>/.cerbo/objects/<uuid>/` (NOT to `page.md` inside it). The target SHALL be expressed as a relative path computed from the symlink's parent directory.

#### Scenario: Relative target from root leaf
- **WHEN** a symlink is created at `<repo-root>/cerbo/home` for object `<uuid>`
- **THEN** its `readlink` value is `../.cerbo/objects/<uuid>/`

#### Scenario: Relative target from nested leaf
- **WHEN** a symlink is created at `<repo-root>/cerbo/notes/rust/ownership` for object `<uuid>`
- **THEN** its `readlink` value is `../../../.cerbo/objects/<uuid>/`
...
- **THEN** `<repo-root>/cerbo/` is unmodified
...
- **THEN** no symlink named `old-name` exists under `<repo-root>/cerbo/`
...
- **AND** `cerbo symlink` is run
- **THEN** no symlink in `<repo-root>/cerbo/` points to the removed UUID

### Requirement: Atomic two-phase swap

The system SHALL build the new tree in a sibling temp directory `<repo-root>/cerbo.tmp-<pid>/`, then atomically rename it into place. If a previous `<repo-root>/cerbo/` exists, it SHALL first be renamed aside to `<repo-root>/cerbo.gc-<pid>/` before the new tree is renamed in, and finally removed.

#### Scenario: First run (no existing tree)
- **WHEN** `<repo-root>/cerbo/` does not exist
- **AND** `cerbo symlink` succeeds
- **THEN** `<repo-root>/cerbo.tmp-<pid>/` is built and renamed to `<repo-root>/cerbo/`
- **THEN** no `cerbo.tmp-*` or `cerbo.gc-*` siblings remain

#### Scenario: Replace existing tree
- **WHEN** `<repo-root>/cerbo/` already exists and passes the safe-wipe check
- **AND** `cerbo symlink` succeeds
- **THEN** the existing tree is renamed to `<repo-root>/cerbo.gc-<pid>/`
- **THEN** `<repo-root>/cerbo.tmp-<pid>/` is renamed to `<repo-root>/cerbo/`
- **THEN** `<repo-root>/cerbo.gc-<pid>/` is removed
- **THEN** no `cerbo.tmp-*` or `cerbo.gc-*` siblings remain
...
Before removing or replacing `<repo-root>/cerbo/`, the system SHALL walk it recursively and verify EVERY entry is either a directory containing only safe entries OR a symlink whose canonicalised target lives under `<repo-root>/.cerbo/objects/`. If any other entry is found (regular file, symlink to elsewhere, device node, etc.), the system SHALL abort with an error listing offending paths and SHALL leave the existing tree untouched.

#### Scenario: All entries are this repository's symlinks
- **WHEN** every leaf under `<repo-root>/cerbo/` is a symlink resolving inside `<repo-root>/.cerbo/objects/`
- **THEN** the safe-wipe check passes
- **THEN** the rebuild proceeds

#### Scenario: Regular file in the tree
- **WHEN** any path under `<repo-root>/cerbo/` is a regular file (not a directory or symlink)
- **THEN** the command exits non-zero
- **THEN** stderr lists that path
- **THEN** `<repo-root>/cerbo/` is unmodified

#### Scenario: Symlink to foreign target
- **WHEN** a symlink under `<repo-root>/cerbo/` points outside `<repo-root>/.cerbo/objects/`
- **THEN** the command exits non-zero
- **THEN** stderr lists that symlink and its target
- **THEN** `<repo-root>/cerbo/` is unmodified

#### Scenario: Symlink to another repository's objects
- **WHEN** a symlink under `<repo-root>/cerbo/` points into a DIFFERENT cerbo repository's `.cerbo/objects/`
...
- **THEN** the leaf in `<repo-root>/cerbo/` is named `rust-ownership`
