...
### Requirement: Add vault via native picker
**Reason**: Replaced by `cerbo init` command for UUID-based storage model.
**Migration**: Users manually run `cerbo init` in their desired directory.

### Requirement: Relocate vault
**Reason**: Vaults are now identified by `.cerbo/` presence, not stored paths.
**Migration**: User can move the entire vault directory; `.cerbo/` moves with it.
