...
- **Repository discovery is mandatory**: like `git`, the command walks up from cwd looking for `.cerbo/`. If none is found, the command aborts with `not a cerbo repository (or any parent up to mount point)`. There is no global-vault-name lookup mode — `cerbo symlink` always operates on the repository the user is currently inside.
