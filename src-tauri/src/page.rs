...
fn vault_root(app: &AppHandle, vault_id: &str) -> Result<PathBuf, String> {
    let reg = load_vaults(app)?;
    let vault = reg
        .vaults
        .iter()
...
fn page_path(root: &PathBuf, slug: &str) -> PathBuf {
...
pub fn page_create(app: AppHandle, vault_id: String, title: String) -> Result<String, String> {
    let root = vault_root(&app, &vault_id)?;
...
mod tests {
    ...
    fn duplicate_slug_rejected() {
        ...
        fs::create_dir_all(&dir).unwrap();
        // Second attempt should detect dir exists
        assert!(dir.exists());
    }
}
